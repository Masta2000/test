  ...
  function generateText(err) {
    ...
    async.each(
      ...
      function(table, _callback) {
        ...
        _.each(fields, function(field, i) {
          ...
          if (self.tables[table][field].type === 'USER-DEFINED' && !!self.tables[table][field].special) {
            if(self.tables[table][field].comment === 'geography'){
                self.tables[table][field].type = "geography"
            }else{
            self.tables[table][field].type =
              'ENUM(' +
              self.tables[table][field].special
                .map(function(f) {
          ...
          _.each(fieldAttr, function(attr, x) {
            var isSerialKey =
              self.tables[table][field].foreignKey &&
            ...
            if (attr === 'foreignKey') {
              if (isSerialKey) {
                ...
                text[table] +=
                  ...
                  spaces +
                  "model: '" +
                  self.tables[table][field][attr].foreignSources.target_table +
                  "',\n";
                text[table] +=
                  spaces +
                  spaces +
                  ...
                  spaces +
                  "key: '" +
                  self.tables[table][field][attr].foreignSources.target_column +
                  "'\n";
                text[table] += spaces + spaces + spaces + '}';
              } else {
                return true;
              }
            } else if (attr === 'primaryKey' || attr === 'autoIncrement') {
              if (
                self.tables[table][field][attr] === true &&
                (!_.has(self.tables[table][field], 'foreignKey') ||
                  (_.has(self.tables[table][field], 'foreignKey') &&
                    !!self.tables[table][field].foreignKey.isPrimaryKey))
              ) {
              ...
              // mssql bit fix
                ...
                self.sequelize.options.dialect === 'mssql' &&
                self.tables[table][field].type.toLowerCase() === 'bit'
              ) {
              ...
              if (defaultVal === null || defaultVal === undefined) {
                ...
                val_text =
                  _.isString(val_text) && !val_text.match(/^sequelize\.[^(]+\(.*\)$/)
      ...
      function() {
        ...
        if (self.options.typescript) {
          typescriptFiles[1] = tsHelper.model.generateTableModels(
            _.keys(text),
            self.options.spaces,
            self.options.indentation,
            self.options.camelCase,
            self.options.camelCaseForFileName
          );
        }
